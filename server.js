...
// $env:GOOGLE_APPLICATION_CREDENTIALS=".\ChatAppKotlin-26a2f336dc10.json"(Windown)
  ...
  async update() {
    ...
    if (this.automode) {
      ...
      for (var area_id in this.areas) {
        var didlist = await this.db.get_device(area_id);
        //console.log(did);
        ...
        didlist.forEach(did => {
          ...
          var humi = (low_humi + high_humi) / 2;
          if (area_humi < low_humi) {
            var messpub = `[\n{ "device_id": "${did}",\n"values":["1", "` + ((humi - area_humi) * 50).toString() + `"]\n}\n]`;
            console.log("publish message : " + messpub)
            this.client.publish('Topic/Speaker',
              messpub);
            this.db.log_device(new Data(did, [1, (humi - area_humi) * 50]));
          }
  ...
  config_mqtt_client(db) {
    ...
    client.on('message', async function (topic, payload) {
      if (topic == 'Topic/TempHumi') {
        ...
        //console.log("message receive" + data_list)
...
class DB {
  ...
  async log(data_list) {
    data_list.forEach(data => {
      ...
      if (data.id.charAt(0) == 'S' || data.id.charAt(0) == 'T') {
        console.log("Add sensor log")
  ...
  async get_area(id) {
    if (id != null) {
      if (id.charAt(0) == 'D' || id == 'Speaker') {
        ...
        console.log("--get success snapshot - log sensor")
        if (snapshot.exists) {
          return snapshot.get('AreaId');
        }
      }
  ...
  async get_device(area_id) {
    ...
    if (area_id != null) {
      ...
      var id_list = [];
      query_snapshot.docs.forEach(doc => id_list.push(doc.id));
      return id_list;

      // if (query_snapshot.docs[0].exists) {
      //   //console.log(query_snapshot.docs.toString())
...
setInterval(update, 10000); // Interval time setting 5' 1 lan
